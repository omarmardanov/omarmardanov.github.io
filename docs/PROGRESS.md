...
- **Контент-команда заказчика начала работу**: выписывают все услуги со старого сайта, переформулируют, готовят отраслевые/судебные лендинги (см. список slug ниже) как обычные услуги.
- **Решение по CMS — новый поворот.** Директор отверг WordPress (не устраивают скорость и сложность) и спросил, «может Claude сделает свой движок». Рекомендация (см. сессию 27 мая): **Astro SSG + лёгкая CMS** — статика на выходе (скорость как сейчас), наши страницы переносятся ~1:1, встроенный i18n под 3 языка, хостинг на том же Beget без PHP/MySQL. Directus не отменяется — может стать «складом» контента под Astro. Бесписьменную CMS с нуля — **не рекомендую** (bus factor, security, дорого жить). Решение Astro vs Directus — за директором.
- **Новое требование: 3 языка (RU / EN / ES).** Визуальный макет переключателя добавлен в топбар (RU активен, EN/ES — заглушки `href="#"`); реальный i18n — вместе с миграцией движка. Открытый вопрос директору: полный перевод всего или EN/ES только для ключевых страниц.
...
2. **Решение директора по движку** (Astro vs Directus) + **по объёму перевода** (всё vs ключевые страницы) → от них зависит путь миграции и реальный i18n.
3. **WP-разработчику создать 10 страниц-услуг** по шаблону `uslugi-shablon/` под slugs (актуально, если победит путь, где страницы создаются вручную):
...
## ▶ Сессия 27 мая 2026 — вопрос движка, требование 3 языков, макет переключателя

Разговор по итогам общения с директором + одна тактическая правка вёрстки.

### 1) Движок — директор отверг WordPress, спросил про «свой движок»

Директора не устраивают скорость и сложность WP. Разобрали по-честному:

- Ключевой факт: **сайт уже статический** → проблема скорости WP по сути снята (WP тормозит из-за PHP+MySQL на каждый запрос). Настоящая задача — редактирование контента + теперь 3 языка, не теряя статической скорости.
- **Бесписьменную CMS с нуля — не рекомендую**: долг на годы (security-патчи, бэкапы, баги), bus factor (поддержит только автор). «Быстро+просто» так не получается.
- **Рекомендация — Astro SSG + лёгкая CMS поверх git** (или Directus как «склад» данных): статика на выходе = скорость как сейчас; наши страницы переносятся ~1:1, дизайн-система (CSS) остаётся; 110 услуг / 45 классов МКТУ = один шаблон + N записей; встроенный i18n-роутинг (`/`, `/en/`, `/es/`); хостинг nginx на том же Beget без PHP/MySQL; Astro — мейнстрим, не самопал.
- Решение **Astro vs Directus** — за директором. Зафиксировали: пока не фиксируем, отношу директору сравнение.

### 2) 3 языка (RU / EN / ES)

- Заказчик попросил 3 языка. Раскладка топбара по согласованию: `[Москва·тел] [Тюмень·тел]` слева — `[Email]` в середине — `[RU EN ES]` справа.
- **Предупреждение по объёму**: 3 языка = весь контент ×3 (юридический EN/ES — не Google Translate). Открытый вопрос директору: полный перевод или EN/ES только для ключевых страниц (главная, 4 направления, о бюро, контакты).
- **Переключатель в подвал — не добавляем** (плотный; стандарт — только в шапке).

### 3) Сделано в вёрстке — макет переключателя языка

- `partials/header.html`: в `.topbar__inner` добавлен `<nav class="topbar__lang">` с RU/EN/ES (RU `is-active`, EN/ES — заглушки `href="#"`); email уехал в середину автоматически за счёт `justify-content: space-between` на трёх детях. CSS-класс `.topbar__lang` уже был заготовлен в `header.css` — новых стилей не понадобилось.
- `partials/mobile-menu.html`: топбар скрыт `< 1024px`, поэтому переключатель продублирован в мобильном меню — последним пунктом блока контактов (`.mobile-menu__lang-row`), сегмент-контролом RU/EN/ES под рядом мессенджеров (та же высота 40px и рамка, активный залит ink'ом). Стили в `mobile-menu.css`.
- Раскатано `scripts/sync-partials.py` на все 19 страниц (топбар + мобильное меню).
