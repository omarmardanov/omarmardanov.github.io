...
## ▶ Что дальше (на 15 мая 2026, обновлено поздним вечером)

**Состояние:** все вёрстко-технические задачи закрыты. Сегодня закрыли 3 крупных блока: гайд для контент-команды (`docs/content-templates/`, 9 файлов), полная WP-инструкция (`docs/WORDPRESS.md` переписан с нуля), `404.html` в стиле hero. Заказчик подберёт замену картинки CTA-фона (без человека) — тогда обновится и 404, и CTA-блок одновременно.

**Что осталось:**

1. **WP-разработчику создать 10 страниц-услуг** по шаблону `uslugi-shablon/` под slugs:
...
2. **WordPress-контент** по `docs/content-templates/` — заполнение услуг, направлений, кейсов, статей, МКТУ, команды по шаблонам.
